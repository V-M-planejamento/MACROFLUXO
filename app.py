...
# --- Bloco de Filtros no Sidebar (com abreviação de empreendimentos) ---

# --- Bloco de Filtros no Sidebar (com a correção do KeyError) ---
...
if df_data is not None and not df_data.empty:
    ...
    with st.sidebar:
        ...
        # 2️⃣ Filtro Empreendimento (com lógica de abreviação)
        if selected_ugb:
            emp_options_completas = get_unique_values(
                df_data[df_data["UGB"].isin(selected_ugb)],
                "Empreendimento"
            )
            
            # Mapeamento de nome abreviado para nome completo
            mapa_abreviado_para_completo = {abreviar_nome(nome): nome for nome in emp_options_completas}
            
            # Opções para o filtro (nomes abreviados)
            emp_options_abreviadas = sorted(list(mapa_abreviado_para_completo.keys()))
            
            # --- INÍCIO DA CORREÇÃO ---
            # O estado padrão do filtro também DEVE usar os nomes abreviados.
            # Se o estado da sessão já existir, precisamos converter os nomes completos
            # que podem estar armazenados lá para nomes abreviados.
            
            # Primeiro, criamos um mapa reverso para facilitar a conversão
            mapa_completo_para_abreviado = {v: k for k, v in mapa_abreviado_para_completo.items()}

            # Verificamos se já existe um estado salvo para este filtro
            if 'empreendimento_filter_selected' in st.session_state:
                # Pega a seleção atual (que pode conter nomes completos da execução anterior)
                selecao_atual_completa = st.session_state['empreendimento_filter_selected']
                # Converte os nomes para a forma abreviada, ignorando os que não estão no mapa
                st.session_state['empreendimento_filter_selected'] = [
                    mapa_completo_para_abreviado[nome] 
                    for nome in selecao_atual_completa 
                    if nome in mapa_completo_para_abreviado
                ]
            
            # O padrão agora é a lista de todos os nomes abreviados.
            default_selected_abreviado = emp_options_abreviadas
            # --- FIM DA CORREÇÃO ---

        else:
            emp_options_abreviadas = []
            mapa_abreviado_para_completo = {}
            default_selected_abreviado = []
            
        # Chama o componente com os nomes abreviados
        selected_emp_abreviado = simple_multiselect_dropdown(
            label="Filtrar por Empreendimento",
            options=emp_options_abreviadas,
            key="empreendimento_filter",
            default_selected=default_selected_abreviado
        )
        
        # Traduz os nomes abreviados selecionados de volta para os nomes completos
        # Esta linha agora funcionará, pois a seleção estará sempre em formato abreviado.
        selected_emp = [mapa_abreviado_para_completo[nome] for nome in selected_emp_abreviado]
        ...
        # 4️⃣ Filtro de Etapas não concluídas
        st.sidebar.markdown("---")
    ...
    with tab1:
        ...
        if df_detalhes.empty:
            ...
            if usar_layout_horizontal:
                ...
                tabela_para_processar['Empreendimento'] = tabela_para_processar['Empreendimento'].apply(abreviar_nome)
                ...
                # --- Bloco de Geração da Tabela Hierárquica (com correção do IndexError) ---

# Agrupa por 'ordem_empreendimento' para manter a ordem correta
                for _, grupo in df_ordenado.groupby('ordem_empreendimento', sort=False):
                
                    # --- INÍCIO DA CORREÇÃO ---
                    # Adiciona uma verificação para garantir que o grupo não esteja vazio
                    ...
                    # --- FIM DA CORREÇÃO ---
                        ...
                        # MODIFICAÇÃO APLICADA AQUI
                        nome_abreviado = abreviar_nome(empreendimento)
                        
                        percentual_medio = grupo['Percentual_Concluido'].mean()
                        
                        cabecalho = pd.DataFrame([{
                            'Hierarquia': f'📂 {nome_abreviado}', # Usando o nome abreviado
